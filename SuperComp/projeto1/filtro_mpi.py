...
if rank != 0:
    ...
    for i in range(1, size):
        ...
        data = req.wait()
    tmp_fim = time.time() - tmp_ini #contabiliza o tempo do arquivo
    tmp_fim = int(tmp_fim * 1000)
    print("Demorou {0} para filtrar".format(tempoIntToStr(tmp_fim)))
#######################################################################
...
fim = time.time() - tmp_ini #contabiliza o horario
fim = int(fim * 1000)
print("Tempo total = {:.3f}".format(tempoIntToStr(fim))) #mostra o tempo que demorou no console